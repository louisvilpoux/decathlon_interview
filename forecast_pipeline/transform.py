...
class Transformer():
    ...
    def check_columns(self):
        ...
        Input :
            Nothing
        Return :
            check_train, check_test : True if the dataframes have the correct
                 column names, False otherwise
        '''
        columns_train = ["day_id", "but_num_business_unit",
                         "dpt_num_department", "turnover"]
        check_train = all(col_df in columns_train
                          for col_df in self.df_train.columns)
        columns_test = ["day_id", "but_num_business_unit",
                        "dpt_num_department"]
        check_test = all(col_df in columns_test
                         for col_df in self.df_test.columns)
    ...
    def convert_column_datetime(self, df, column_name, format="%Y-%m-%d"):
        ...
        Input :
            ...
            column_name : the name of the column to convert to a datetime
                            column
    ...
    def add_year_column(self, df, date_column):
        ...
        Input :
            df : the input dataframe
            date_column : the column from the dataframe to use to calculate
    ...
    def representation_businessunit_from_year(self):
        '''
        Function to obtain the mean turnover percentage of the different
         business units from the annual turnover
        Input :
            Nothing
        Return :
            df_businessunit_mean : dataframe of the percentages to apply
    ...
    def representation_department_from_businessunit(self):
        '''
        Function to obtain the mean turnover percentage of the departments
         from the business units
        Input :
            Nothing
        Return :
            df_department_mean : dataframe of the percentages to apply for
                                    each department
        '''
    ...
    def create_threshold_date(self, df):
        '''
        Function to obtain a date that was duration_predict days from the
        maximum date of a dataframe
        Input :
            df : initial dataframe
        Return :
    ...
    def preprocess(self):
        '''
        Function to obtain the percentage representation of the turnover for
        the different granularity
        Input :
            Nothing
        Return :
            self.df_train : train dataframe
            df_train_model : train dataframe for the model
            df_test_model : validation dataframe for the model
            self.df_test : test dataframe
            df_businessunit_repr : dataframe of the representation of the
                                    business units
            df_department_repr : dataframe of the representation of the
                                    departments
            df_week_department_repr : dataframe of the representation of the
                                    weeks
            limit_date : the date used to split train and validate dataframes
        '''
        check_train, check_test = self.check_columns()
        if not check_train:
            self.logger.logger.error(f"Train data do not have the right \
            columns. End of the program.")
            exit()
        if not check_test:
            self.logger.logger.error(f"Test data do not have the right \
            columns. End of the program.")
            exit()
        try:
            ...
            return self.df_train, df_train_model, df_test_model, \
                self.df_test, df_businessunit_repr, df_department_repr, \
                    df_week_department_repr, limit_date
